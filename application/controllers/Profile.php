...
class Profile extends My_Controller
{
    ...
    public function profile_mahasiswa()
    {
        ...
        $this->paggingPembeli('pembeli/profile/profile_mahasiswa', $data);
    }
    ...
    public function edit_profile_mahasiswa()
    {
        ...
        //jika form validasi salah
        if ($this->form_validation->run() == FALSE) {
            $this->paggingPembeli('pembeli/profile/edit_profile_mahasiswa', $data);
        } else {
            $this->user_model->editProfile_mahasiswa();
            $this->session->set_flashdata('message', '<div class="flash-data" data-flashdata="Data Berhasil di Ubah"></div>');
            redirect('profile/profile_mahasiswa');
        }
    }
    ...
    public function ubah_password_mahasiswa()
    {
        ...
        //jika form validasi salah
        if ($this->form_validation->run() == FALSE) {
            $this->paggingPembeli('pembeli/profile/ubah_password_mahasiswa', $data);
        } else {
            $password_lama = $this->input->post('pw_lama');
            $password_baru = $this->input->post('pw_baru');
            if (!password_verify($password_lama, $data['mahasiswa']['password_mahasiswa'])) {
                $this->session->set_flashdata('message', '<div class="alert alert-danger" role="alert">Password lama salah</div>');
                redirect('profile/ubah_password_mahasiswa');
            } else {
                if ($password_lama == $password_baru) {
                    $this->session->set_flashdata('message', '<div class="alert alert-danger" role="alert">Password baru tidak boleh sama dengan password lama</div>');
                    redirect('profile/ubah_password_mahasiswa');
                    ...
                    //password OK
                    $password_hash = password_hash($password_baru, PASSWORD_DEFAULT);
                    $this->db->set('password_mahasiswa', $password_hash);
